...
int keypress(XEvent event){
 ...
 int main(int argc, char* argv[])
 {
    ...
    attr.event_mask = ButtonPressMask|ExposureMask|KeyPressMask|PointerMotionMask;
    ...
    Window win =  XCreateWindow(display, DefaultRootWindow(display) ,0, 0, width, height, 0, 
			...
			for(int i=0;i<64;i++){
				for(int j=0;j<64;j++){
					...
					XSetForeground(display,gc, 0x80000000);
			    		XFillRectangle(display, win, gc, awidth*i, aheight*j,awidth, aheight);
					XSetForeground(display,gc, 0x80ffffff);
					    char msg[4] = {candidates[level1], candidates[level2], candidates[level3],'\0'};
					    /*char msg[4] = {candidates[level1], '\0'};*/
				    	XDrawString(display, win, gc, awidth*i, aheight*j, msg, strlen(msg));
				}
			}
			break;
		case KeyPress:
