...
int
drw_text_x(Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad, const char *text, int invert, int hasbackground)
{
	char buf[1024];
	int ty;
	unsigned int ew;
...
{
	return drw_text_x(drw, x, y, w, h, lpad, text, invert, 1);
}
